...
weather_file = 'NLD_OV_Heino.062780_TMYx2007-2021/Heino.cli'
...
### ends Global functions definition ####

### Source code ###

### Part 2: Download geometry data from the 3DCityDB ###
scenarios = ['statusQuo','mediumRefurb','advRefurb']
for refurb_scenario in scenarios:
    #### Output files ####
    output_folder = '../data'
    output_citysimfile = '{}/{}_Group_02_{}.xml'.format(output_folder, input_city, refurb_scenario)
    ...
    tree_envelope = gpd.read_postgis(f'''SELECT g.* 
                                     from (
                                     SELECT o.id::bigint AS co_id, 
                                     ST_SetSRID(ST_Affine(ST_Collect(sg.implicit_geometry),split_part(lod2_implicit_transformation, ' ', 1)::numeric,0,0,0,split_part(lod2_implicit_transformation, ' ', 6)::numeric,0,0,0,split_part(lod2_implicit_transformation, ' ', 11)::numeric,ST_X(o.lod2_implicit_ref_point),ST_Y(o.lod2_implicit_ref_point),ST_Z(o.lod2_implicit_ref_point)),srs.srid)::geometry(MultiPolygonZ, 28992) AS geom 
                                     FROM {e1_dbschema}.solitary_vegetat_object AS o 
                                     INNER JOIN {e1_dbschema}.implicit_geometry AS ig 
                                     ON (ig.id = o.lod2_implicit_rep_id) 
                                     INNER JOIN {e1_dbschema}.surface_geometry AS sg 
                                     ON (sg.root_id = ig.relative_brep_id), 
                                     {e1_dbschema}.database_srs AS srs 
                                     GROUP BY o.id,srs.srid)g''',
                                     db_engine1, geom_col = "geom")
    # Download terrain geometry envelope from the database
    terrain_envelope = gpd.read_postgis("SELECT surface_geometry.gmlid, surface_geometry.id, surface_geometry.geometry FROM surface_geometry, tin_relief WHERE surface_geometry.parent_id = tin_relief.surface_geometry_id", db_engine1, geom_col = "geometry")
    ...
    Wall_type_windowTypeId_df = pd.read_sql_query("select library,country,type as building_type,function,year_from, year_to,opening_id as outWalls_windowTypeId from building.v_parameter where element = 'outerWall' and refurbishment = '{}' and name = 'window' and country='{}';".format(refurb_scenario,country), db_engine2)
    ...
    Wall_type_windowRatio_df = pd.read_sql_query("select library,country,type as building_type,function,year_from, year_to,value as outWalls_windowRatio from building.v_parameter where element = 'outerWall' and refurbishment = '{}' and name = 'windowRatio' and country='{}';".format(refurb_scenario,country), db_engine2)
    ...
    Wall_type_uValue_df = pd.read_sql_query("select library,country,type as building_type,function,year_from, year_to,value as outWalls_uValue from building.v_parameter where element = 'outerWall' and refurbishment = '{}' and name = 'uValue' and country='{}';".format(refurb_scenario,country), db_engine2)
    ...
    Ground_type_uValue_df = pd.read_sql_query("select library,country,type as building_type,function,year_from, year_to,value as groundShell_uValue from building.v_parameter where element = 'groundShell' and refurbishment = '{}' and name = 'uValue' and country='{}';".format(refurb_scenario,country), db_engine2)
    if Ground_type_constructionTypeId_df.shape[0] == 0:
    ...
    Roof_type_windowTypeId_df = pd.read_sql_query("select library,country,type as building_type,function,year_from, year_to,opening_id as pitchedRoof_windowTypeId from building.v_parameter where element = 'pitchedRoof' and refurbishment = '{}' and name = 'window' and country='{}';".format(refurb_scenario,country), db_engine2)
    ...
    Roof_type_uValue_df = pd.read_sql_query("select library,country,type as building_type,function,year_from, year_to,value as pitchedRoof_uValue from building.v_parameter where element = 'pitchedRoof' and refurbishment = '{}' and name = 'uValue' and country='{}';".format(refurb_scenario,country), db_engine2)
    ...
    shading_list = gpd.read_postgis(f'''select sg.id, c.gmlid, sg.geometry from {e1_dbschema}.thematic_surface as ts 
                                    inner JOIN {e1_dbschema}.surface_geometry as sg 
                                    ON ts.lod2_multi_surface_id = sg.parent_id 
                                    inner JOIN {e1_dbschema}.cityobject as c 
                                    ON ts.building_id = c.id
                                    WHERE c.gmlid in {shading_list_bldg}''', 
                                    db_engine1, geom_col = "geometry").set_crs(epsg=28992, inplace=True)
    
    # time_delta(time_start)
    # sys.exit()
    
    ### Part 5. Party walls processing ###
    ...
    ### Part 6. Check surfaces that will be simulated are convex or not. If not, it tries to triangulate them. ###
    
    # remove buildings that shall not be processes because their geometries are not valid
    ...
    print(f'Residential Buildings in the study area: {building_list.shape[0]}')
    ...
    print(f'Residential Buildings to simulate: {building_list.shape[0]}')
    ...
    print('Ends data processing. Begins CitySim XML file creation')
    ...
    text = text + '<FarFieldObstructions>\n'
    hor = open('../data/Rijssen.hor','r')
    lines = hor.readlines()
    for line in lines:
        i = line.split('	')
        i[-1] = i[-1].strip()
        text = text + ('<Point phi=\"'+i[0]+'\"'+" "+'theta=\"'+i[1]+'\"/>\n')
    hor.close()
    text = text + '</FarFieldObstructions>\n'
